...
`;
  ...
  // 19. Create AGENTS.md
  createAgentsMd(projectDir, projectName);
}
...
function createAgentsMd(projectDir: string, projectName: string) {
...
1. **Define the model**: \`npx swallowkit create-model <name>\`
   - Creates \`shared/models/<name>.ts\` with a Zod schema template.
   - Edit the generated file to add your fields.

2. **Generate CRUD**: \`npx swallowkit scaffold shared/models/<name>.ts\`
   - Generates Azure Functions handlers, BFF routes, UI pages, and Cosmos DB Bicep config.

3. **Start development**: \`npx swallowkit dev\`
   - Runs Next.js, Azure Functions, and checks for Cosmos DB Emulator concurrently.
