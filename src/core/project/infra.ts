...
export function inspectInfra(projectRoot: string = process.cwd()): InfraInspection {
  ...
  const migrationManifestPath = path.join(infraDir, "migrations", "manifest.json");
  let migrationVersions: number[] = [];
  if (fs.existsSync(migrationManifestPath)) {
    try {
      const manifest = JSON.parse(fs.readFileSync(migrationManifestPath, "utf-8")) as { migrations?: Array<{ version?: number }> };
      migrationVersions = (manifest.migrations ?? [])
        .map((migration) => migration.version)
        .filter((version): version is number => Number.isSafeInteger(version))
        .sort((left, right) => left - right);
    } catch {
      warnings.push("migration-manifest-invalid: infra/migrations/manifest.json is not valid JSON.");
    }
  }
  ...
  return {
    ...
    containers,
    migrations: {
      manifestExists: fs.existsSync(migrationManifestPath),
      versions: migrationVersions,
    },
    boundary: {
      zone: "shared",
      note: "infra/main.bicep and modules form the baseline. New model containers and custom resources belong in immutable infra/migrations entries.",
    },
    warnings,
  };
}
