...
When `scaffold` processes a model that has an `authPolicy` (either exported from the model file or defined in `auth.authorization.policies`), it injects role guards into the generated Functions code.
