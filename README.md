...
Projects generated by `swallowkit init` now also include a project-scoped `.mcp.json` bootstrap. Agent runtimes that support repository MCP discovery can start using the same `swallowkit_*` tools automatically from the project root.
